...
def _extract_grayscale(img, min_face_size):
    ...
    if _has_green_border(img):
        logger.info("Green border detected — using CLAHE grayscale path")
...
def extract_face_masks(img, min_face_size=500, dilation_radius=1,
    ...
    logger.debug("Stage 1a — luminosity=%.1f (threshold=%s), min_face_size=%s",
                 lum, lum_fallback_thresh, min_face_size)
    if lum < lum_fallback_thresh:
        logger.debug("Stage 1a — segmentation path: HSV (lum < threshold)")
        _used_hsv = True
        ...
        if len(valid) >= 3:
            _used_hsv = False
            logger.debug("Stage 1a — segmentation path: grayscale")
        else:
            logger.warning("Stage 1a — grayscale yielded only %d face(s); falling back to HSV", len(valid))
            _used_hsv = True
            labeled, valid, fpix = _extract_hsv(img, min_face_size)

    if len(valid) > 0:
        sizes = [len(fpix[f]) for f in valid]
        logger.info("Stage 1a — %d faces detected | sizes min/med/max = %d/%d/%d",
                    len(valid), min(sizes), int(np.median(sizes)), max(sizes))
    else:
        logger.warning("Stage 1a — no faces detected")
...
def refine_faces(img, labeled_low, face_pixels_low, valid_low,
    ...
    logger.debug("Stage 1b — refine_faces: path=%s, faces before=%s", path, n_before)
    if _used_hsv:
    ...
    try:
        _, valid_after, _ = out
        logger.info("Stage 1b — %d faces after refinement", len(valid_after))
    except Exception:
        pass
    return out
...
def extract_face_corners(labeled_clean, face_pixels, tolerance=4):
    ...
    if counts:
        skipped_note = f" | skipped (no contour): {skipped_no_contour}" if skipped_no_contour else ""
        logger.info("Stage 2a — %d raw corners across %d faces | per-face min/med/max = %d/%d/%d%s",
                    sum(counts), len(face_corners),
                    min(counts), int(np.median(counts)), max(counts), skipped_note)
    return face_corners
...
def filter_corners(face_corners, labeled_clean, valid_labels, radius=15):
    ...
    logger.info("Stage 2b — dropped %d interior corners from %d faces → %d remain",
                dropped_total, affected, total_after)
    return filtered
...
def filter_flat_corners(face_corners, labeled_clean, valid_labels,
    ...
    for fid in valid_labels:
        ...
        if len(kept) < min_keep and K >= min_keep:
            if fid in debug_set:
                logger.debug("Stage 2c — face %d: would drop to %d corners (< min_keep=%d); keeping originals. angles=%s",
                             fid, len(kept), min_keep,
                             ['%.1f' % a if a is not None else 'n/a' for a in angles])
        ...
        if fid in debug_set:
            for k, pt in enumerate(ordered_corners):
                ...
                logger.debug("Stage 2c — face %d corner @ (%.1f,%.1f) angle=%s° -> %s",
                             fid, pt[0], pt[1], ang_s, tag)
        for pt, ang, bs, fs in dropped_log:
            logger.debug("Stage 2c — face %d: dropping flat corner @ (%.1f,%.1f) angle=%.1f° (back=%d, fwd=%d)",
                         fid, pt[0], pt[1], ang, bs, fs)
    ...
    total_after = sum(len(v) for v in filtered.values())
    logger.info("Stage 2c — dropped %d flat corners from %d faces → %d remain",
                total_dropped, affected_faces, total_after)
    return filtered
...
def compute_adjacency(labeled_clean, valid_labels, face_pixels,
        ...
        for lbl, cnt in zip(labels, counts):
            if lbl in label_to_idx and lbl != i:
                ...
                for di, dj in debug_pairs:
                    if i == di and lbl == dj:
                        logger.debug("  Border %d→%d: %d pixels", i, lbl, cnt)
            ...
            for di, dj in debug_pairs:
                if (i == di and lbl == dj) or (i == dj and lbl == di):
                    ...
                    if blockers and mask_lbl.any():
                        ...
                        logger.debug("  [BARRIER] %d→%d: blocked by %s (sample coords: %s)",
                                     i, lbl, sorted(blockers), sample_pts)
                    else:
                        logger.debug("  [BARRIER] %d→%d: raw border count %d, BFS did not reach — reachable=%s, disk_border=%s",
                                     i, lbl, adjacency[a, b],
                                     sorted(reachable_labels), sorted(disk_border_labels))
    ...
    if drop.any():
        for a, b in zip(*np.where(drop)):
            if a < b:
                ...
                prev = adjacency[a, b]
                logger.debug("Adjacency dropped: pair %d-%d (raw count=%d, blocked both directions)",
                             i_lbl, j_lbl, prev)
    ...
    for di, dj in debug_pairs:
        if di in label_to_idx and dj in label_to_idx:
            ...
            if one_sided[a, b] or one_sided[b, a]:
                logger.debug("Pair %d-%d: one-sided barrier block (blocked[%d→%d]=%s, blocked[%d→%d]=%s) — kept",
                             di, dj, di, dj, blocked[a, b], dj, di, blocked[b, a])
            logger.debug("Pair %d-%d: border count (symmetrized)=%d | final=%d, threshold=%d",
                         di, dj, adjacency[a, b], adjacency[a, b], shared_border_threshold)
    ...
    for a, i in enumerate(valid_labels):
        ...
        for b in range(a + 1, n):
            ...
            if is_debug:
                ...
                logger.debug("Centroid-line %d-%d: raw_count=%d, runs=%s, sandwiched-blockers=%s",
                             i, j, raw_count, run_brief, sorted(blockers))
            ...
            if raw_count >= strong_border_cutoff:
                logger.debug("Pair %d-%d: centroid line crosses %s but raw count %d >= strong cutoff %d — keeping",
                             i, j, sorted(blockers), raw_count, strong_border_cutoff)
                continue

            logger.debug("Adjacency dropped: pair %d-%d — centroid line crosses %s (raw count=%d)",
                         i, j, sorted(blockers), raw_count)
            adjacency[a, b] = 0
            adjacency[b, a] = 0
    ...
    if degrees:
        logger.info("Stage 3 — %d adjacency pairs accepted (threshold=%d) | per-face degree min/med/max = %d/%d/%d",
                    accepted_pairs, shared_border_threshold,
                    min(degrees), int(np.median(degrees)), max(degrees))
    else:
        logger.warning("Stage 3 — no faces found during adjacency computation")
...
def merge_vertices(face_corners, valid_labels, adjacency, face_pixels,
    ...
    logger.info("Stage 4 — merging vertices: %d faces, %d raw corners, %d bulbs, %d adjacency edges",
                len(valid_labels), len(all_pts),
                0 if bulbs is None else len(bulbs),
                int(adjacency.sum() // 2))
    ...
    next_vid = 1
    MAX_CORNERS = 4
    DEBUG_FACES = [11,10,14,14,29,32]  # face to trace
        ...
        for bi in range(len(bulb_arr)):
            ...
            for fid in faces_here:
                for idx in face_pt_indices[fid]:
                    ...
                    if idx in pt_to_vertex:
                        continue
                    if np.linalg.norm(all_pts[idx] - bulb_pos) <= dilation_radius:  # extra fudge to catch stragglers
            ...
            for fid in faces_here:
                ...
                if fid in DEBUG_FACES:
                    logger.debug("[BORN] vid=%d @ %s | stage=6a-upfront-bulb bi=%d | face=%d | faces_here=%s",
                                 next_vid, bulb_pos, bi, fid, faces_here)
            used_bulbs.add(bi)
    ...
    cycle_vids: set[int] = set()  # vertices born in stage 6-cycle; immune to 7c merge
    DEBUG_CYCLE = set([23,24,26,29])
    for cycle in cycles:
        _dbg_cycle = DEBUG_CYCLE and DEBUG_CYCLE.issubset(set(cycle))
        if len(cycle) > 6:
            if _dbg_cycle:
                logger.debug("[CYCLE-DROP] cycle=%s: len > 6", cycle)
            continue
        ...
        for fid in cycle[1:]:
            ...
            if _dbg_cycle:
                logger.debug("[CYCLE-INTERSECT] cycle=%s | fid=%d | inter_px=%d | trial_px=%d | %s",
                             cycle, fid, inter.sum(), trial.sum(), "KEPT" if trial.any() else "DROPPED")
            if trial.any():
                inter = trial
                surviving.append(fid)
        if len(surviving) < 2:
            if _dbg_cycle:
                logger.debug("[CYCLE-DROP] cycle=%s: surviving=%d < 2", cycle, len(surviving))
            continue
        dropped = [fid for fid in cycle if fid not in surviving]
        # Only tolerate a single regressed face. More than one means the
        # cycle has no common junction point (e.g. inner-circle ring) — skip.
        if len(dropped) > 1:
            if _dbg_cycle:
                logger.debug("[CYCLE-DROP] cycle=%s: dropped=%s > 1", cycle, dropped)
            continue
        if _dbg_cycle:
            logger.debug("[CYCLE-OK] cycle=%s | surviving=%s | dropped=%s", cycle, surviving, dropped)
        cycle = surviving

        relevant_indices = np.concatenate([face_pt_indices[fid] for fid in cycle])
        if len(relevant_indices) == 0:
            if _dbg_cycle:
                logger.debug("[CYCLE-SKIP] cycle=%s | no relevant indices", cycle)
            continue
        ...
        if len(bulb_arr) > 0:
            ...
            if _dbg_cycle:
                all_hits = np.where(inside)[0]
                logger.debug("[CYCLE-BULB] cycle=%s | in_inter=%s | after_filter=%s",
                             cycle, list(all_hits), list(hits))
            if len(hits) == 1:
        ...
        if vertex_pos is None:
            def _corners_in_mask(mask, indices):
                result = []
                for idx in indices:
                    idx = int(idx)
                    r, c = all_pts[idx]
                    ri, ci2 = int(round(r)), int(round(c))
                    if 0 <= ri < H and 0 <= ci2 < W and mask[ri, ci2]:
                        result.append(idx)
                return result

            corner_in_inter = _corners_in_mask(inter, relevant_indices)
            if not corner_in_inter:
                # Intersection zone is too small to contain any polygon corner —
                # dilate progressively until we find one or give up.
                search_mask = inter.astype(np.uint8)
                for dil_r in (5, 10, 20):
                    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (dil_r*2+1, dil_r*2+1))
                    search_mask = cv2.dilate(search_mask, kernel)
                    corner_in_inter = _corners_in_mask(search_mask.astype(bool), relevant_indices)
                    if _dbg_cycle:
                        logger.debug("[CYCLE-DILATE] cycle=%s | dil_r=%d | found %d corners",
                                     cycle, dil_r, len(corner_in_inter))
                    if corner_in_inter:
                        break

            if _dbg_cycle:
                logger.debug("[CYCLE-FALLBACK] cycle=%s | corners_found=%d | inter_px=%d",
                             cycle, len(corner_in_inter), inter.sum())
            if not corner_in_inter:
                if _dbg_cycle:
                    logger.debug("[CYCLE-SKIP] cycle=%s | no corners found even after dilation", cycle)
                continue
        ...
        if _dbg_cycle:
            logger.debug("[CYCLE-VERTEX] vid=%d @ %s | from_bulb=%s | winners=%s",
                         next_vid, vertex_pos, vertex_from_bulb, winners)
            for fid in cycle:
                over = len(face_merged_vids[fid]) >= MAX_CORNERS
                logger.debug("  face %d: vids=%d/%d%s", fid, len(face_merged_vids[fid]), MAX_CORNERS,
                             " << WILL SKIP" if over else "")

        vertices[next_vid] = vertex_pos
        cycle_vids.add(next_vid)
        for idx in winners:
            pt_to_vertex[idx] = next_vid
        ...
        for fid in cycle:
            ...
            if len(face_merged_vids[fid]) >= MAX_CORNERS:
                if fid in DEBUG_FACES or _dbg_cycle:
                    logger.debug("[SKIP] vid=%d | stage=6-cycle | face %d at MAX_CORNERS", next_vid, fid)
                continue
            face_merged_vids[fid].add(next_vid)
            if fid not in winner_faces:
                face_extra_vids[fid].append(next_vid)
            if fid in DEBUG_FACES or _dbg_cycle:
                logger.debug("[BORN] vid=%d | stage=6-cycle | face=%d | from_bulb=%s | extra=%s",
                             next_vid, fid, vertex_from_bulb, fid not in winner_faces)
        ...
        for fid in dropped:
            if len(face_merged_vids[fid]) >= MAX_CORNERS:
                if fid in DEBUG_FACES:
                    logger.debug("[SKIP] vid=%d | stage=6-cycle-dropped | face %d at MAX_CORNERS", next_vid, fid)
                continue
            face_merged_vids[fid].add(next_vid)
            face_extra_vids[fid].append(next_vid)

            if fid in DEBUG_FACES:
                logger.debug("[BORN] vid=%d | stage=6-cycle-dropped | face=%d", next_vid, fid)
    ...
    # A real boundary cycle closes through the image edge, so both endpoint
    # faces must actually *touch* that edge. Without this guard, two faces
    # separated only by a degenerate background gap (e.g. from a regressed
    # face) can pass the dilated-mask intersection test in the middle of
    # the image and spawn a bogus junction vertex on internal faces.
    BOUNDARY_TOL = 2
    image_boundary_faces = set()
    for fid in valid_labels:
        pix = face_pixels[fid]
        if len(pix) == 0:
            continue
        if (pix[:, 0].min() <= BOUNDARY_TOL
                or pix[:, 0].max() >= max_r - BOUNDARY_TOL
                or pix[:, 1].min() <= BOUNDARY_TOL
                or pix[:, 1].max() >= max_c - BOUNDARY_TOL):
            image_boundary_faces.add(fid)
    ...
    for a_idx, u_face in enumerate(valid_list):
        for v_face in valid_list[a_idx + 1:]:
            ...
            if u_face == v_face:
                continue
            # Both endpoints must touch the image boundary; otherwise the
            # "closing edge through the image edge" premise doesn't hold.
            if (u_face not in image_boundary_faces
                    or v_face not in image_boundary_faces):
                continue
    ...
    for cycle in boundary_cycles:
        ...
        for fid in cycle:
            if len(face_merged_vids[fid]) >= MAX_CORNERS:
                if fid in DEBUG_FACES:
                    logger.debug("[SKIP] vid=%d | stage=6b-boundary-cycle | face %d at MAX_CORNERS", next_vid, fid)
                continue
            face_merged_vids[fid].add(next_vid)
            if fid not in winner_faces:
                face_extra_vids[fid].append(next_vid)
            if fid in DEBUG_FACES:
                logger.debug("[BORN] vid=%d | stage=6b-boundary-cycle | face=%d | from_bulb=%s | extra=%s",
                             next_vid, fid, vertex_from_bulb, fid not in winner_faces)
        ...
        for fid in dropped:
            if len(face_merged_vids[fid]) >= MAX_CORNERS:
                if fid in DEBUG_FACES:
                    logger.debug("[SKIP] vid=%d | stage=6b-boundary-cycle-dropped | face %d at MAX_CORNERS", next_vid, fid)
                continue
            face_merged_vids[fid].add(next_vid)
            face_extra_vids[fid].append(next_vid)
            if fid in DEBUG_FACES:
                logger.debug("[BORN] vid=%d | stage=6b-boundary-cycle-dropped | face=%d", next_vid, fid)
    ...
    for u, v in G.edges():
        ...
        if len(bulb_arr) > 0:
            ...
            for bi in np.where(inside)[0]:
                ...
                if len(face_merged_vids[u]) < MAX_CORNERS:
                    ...
                    if u in DEBUG_FACES:
                        logger.debug("[BORN] vid=%d | stage=7-edge-bulb | edge (%d,%d) | face=%d", next_vid, u, v, u)
                elif u in DEBUG_FACES:
                    logger.debug("[SKIP] vid=%d | stage=7-edge-bulb | face %d at MAX_CORNERS", next_vid, u)
                if len(face_merged_vids[v]) < MAX_CORNERS:
                    face_merged_vids[v].add(next_vid)
                    if v in DEBUG_FACES:
                        logger.debug("[BORN] vid=%d | stage=7-edge-bulb | edge (%d,%d) | face=%d", next_vid, u, v, v)
                elif v in DEBUG_FACES:
                    logger.debug("[SKIP] vid=%d | stage=7-edge-bulb | face %d at MAX_CORNERS", next_vid, v)
        ...
        while cand_u and cand_v:
            ...
            # Snap to an existing nearby vertex (triple-junction case: two
            # edges meeting at the same topological corner were processed
            # separately, so the earlier edge already owns a vertex here).
            snap_vid = None
            if vertices:
                vids_arr = np.array(list(vertices.keys()), dtype=int)
                pos_arr  = np.array([vertices[k] for k in vids_arr])
                dists    = np.linalg.norm(pos_arr - midpoint, axis=1)
                nearest  = int(np.argmin(dists))
                if dists[nearest] <= dilation_radius:
                    snap_vid = int(vids_arr[nearest])

            if snap_vid is not None:
                use_vid = snap_vid
                pt_to_vertex[ui] = use_vid
                pt_to_vertex[vi] = use_vid
                if u in DEBUG_FACES or v in DEBUG_FACES:
                    logger.debug("[SNAP-REUSE] stage=7-edge-greedy | edge (%d,%d) | snapped to vid=%d dist=%.1f",
                                 u, v, use_vid, dists[nearest])
            else:
                use_vid = next_vid
                vertices[use_vid] = midpoint
                pt_to_vertex[ui] = use_vid
                pt_to_vertex[vi] = use_vid

            if len(face_merged_vids[u]) < MAX_CORNERS:
                face_merged_vids[u].add(use_vid)
                if u in DEBUG_FACES:
                    logger.debug("[BORN] vid=%d | stage=7-edge-greedy | edge (%d,%d) | face=%d", use_vid, u, v, u)
            elif u in DEBUG_FACES:
                logger.debug("[SKIP] vid=%d | stage=7-edge-greedy | face %d at MAX_CORNERS", use_vid, u)
            if len(face_merged_vids[v]) < MAX_CORNERS:
                face_merged_vids[v].add(use_vid)
                if v in DEBUG_FACES:
                    logger.debug("[BORN] vid=%d | stage=7-edge-greedy | edge (%d,%d) | face=%d", use_vid, u, v, v)
            elif v in DEBUG_FACES:
                logger.debug("[SKIP] vid=%d | stage=7-edge-greedy | face %d at MAX_CORNERS", use_vid, v)
    ...
    if len(bulb_arr) > 0:
        ...
        for bi in range(len(bulb_arr)):
            ...
            for fid in faces_here:
                ...
                logger.debug("[BORN] vid=%d | stage=7b-isolated-bulb bi=%d | face=%d | faces_here=%s",
                             next_vid, bi, fid, faces_here)
            used_bulbs.add(bi)
            next_vid += 1
    ...
    if len(vertices) > 1:
        ...
        for i, vi in enumerate(vert_ids):
            ...
            nearby = [vert_ids[j] for j in range(len(vert_ids))
                      if j != i and dists[j] <= dilation_radius +1
                      and vert_ids[j] not in merge_map
                      and vert_ids[j] not in cycle_vids]
            for vj in nearby:
            ...
            for old_vid, new_vid in merge_map.items():
                ...
                for fid in valid_labels:
                    if old_vid in face_merged_vids[fid]:
                        ...
                        logger.debug("[MERGE] stage=7c-nearby-merge | face %d: vid %d -> %d", fid, old_vid, new_vid)
        ...
        for idx in range(len(all_pts)):
            ...
            if d[j] <= dilation_radius:
                ...
                if pt_to_face[idx] in DEBUG_FACES:
                    logger.debug("[SNAP] stage=8-snap | face %d: idx=%d snapped to vid=%d dist=%.1f",
                                 pt_to_face[idx], idx, vert_ids[j], d[j])
    ...
    for idx in range(len(all_pts)):
        ...
        if best_vid is not None and best_dist <= 2 * dilation_radius:
            ...
            logger.debug("[SNAP] stage=8b-topo-snap | face %d: idx=%d snapped to vid=%d dist=%.1f",
                         fid, idx, best_vid, best_dist)
    ...
    for u, v in G.edges():
        ...
        while cu and cv:
            ...
            # Saturation guard: a shared edge between two faces should
            # have exactly 2 endpoint vertices. If (u,v) already has ≥2
            # shared vertices, don't mint a 3rd — snap ui and vi to
            # whichever existing shared vertex is closest to `mid`.
            # Fixes the hole_066 case where stage 6-cycle already placed
            # vids 30 and 36 on edge (28,29) and 8c then spawned a
            # spurious vid 139 ~20px from vid 36.
            shared_vids = [sv for sv in (face_merged_vids[u] & face_merged_vids[v])
                           if sv in vertices]
            if len(shared_vids) >= 2:
                shared_arr = np.array([vertices[sv] for sv in shared_vids])
                d_shared = np.linalg.norm(shared_arr - mid, axis=1)
                k = int(np.argmin(d_shared))
                target = shared_vids[k]
                pt_to_vertex[ui] = target
                pt_to_vertex[vi] = target
                if u in DEBUG_FACES or v in DEBUG_FACES:
                    logger.debug("[SNAP] stage=8c-saturated | edge (%d,%d) already has %d shared vids — snapped to vid=%d dist=%.2f",
                                 u, v, len(shared_vids), target, d_shared[k])
            ...
            face_merged_vids[v].add(next_vid)
            if u in DEBUG_FACES or v in DEBUG_FACES:
                logger.debug("[BORN] vid=%d | stage=8c-unclaimed-pair | edge (%d,%d)", next_vid, u, v)
            cu = [x for x in cu if x not in pt_to_vertex]
            cv = [x for x in cv if x not in pt_to_vertex]
            unclaimed_by_face[u] = cu
            unclaimed_by_face[v] = cv
    ...
    for u, v in G.edges():
        ...
        for svid in list(one_sided):
            ...
            shared.add(svid)
            if u in DEBUG_FACES or v in DEBUG_FACES:
                logger.debug("[PROMOTE] edge (%d,%d): vid=%d added to face %s (shared=%d/2)",
                             u, v, svid, added_to, len(shared))
        if len(shared) >= 2:
            continue
        ...
        if (len(face_merged_vids[u]) >= MAX_CORNERS
            ...
            continue
        if red_mask is None:
            logger.debug("Unsaturated edge (%d,%d): no image for red-border fallback", u, v)
            continue
        red_in_strip = inter_uv & red_mask
        if not red_in_strip.any():
            logger.debug("Unsaturated edge (%d,%d): no red border in dilated intersection", u, v)
            continue
        ...
        if not np.isfinite(cost).any():
            logger.debug("Unsaturated edge (%d,%d): all red pixels too close to existing vertices", u, v)
            continue
        ...
        if u in DEBUG_FACES or v in DEBUG_FACES:
            logger.debug("[BORN] vid=%d | stage=8d-red-border | edge (%d,%d)", next_vid, u, v)
        next_vid += 1
    ...
    for idx in range(len(all_pts)):
        if idx not in pt_to_vertex:
            ...
            logger.debug("[BORN] vid=%d | stage=9-singleton | face=%d | idx=%d", next_vid, fid, idx)
            next_vid += 1

    # 9b. Global 2px vertex merge. Any two vertices within 2px of each
    # other are collapsed, with the lower vid winning (earlier stages are
    # higher-confidence: cycles/edges/bulbs < snaps < singletons).
    # Fixes duplicate vertices at bulbs (issues.md #3,8,9,24,41) where
    # stage 8c/8d/9 spawn a near-duplicate of an existing cycle/edge vid.
    if len(vertices) > 1:
        sorted_vids = sorted(vertices.keys())
        kept = []  # list of (vid, pos) for vertices we're keeping
        for cur_vid in sorted_vids:
            cur_pos = vertices[cur_vid]
            target = None
            if kept:
                kept_arr = np.array([p for _, p in kept])
                d = np.linalg.norm(kept_arr - cur_pos, axis=1)
                j = int(np.argmin(d))
                if d[j] <= 2.0:
                    target = kept[j][0]
                    merge_dist = float(d[j])
            if target is None:
                kept.append((cur_vid, cur_pos))
                continue
            # Merge cur_vid -> target
            for fid in valid_labels:
                if cur_vid in face_merged_vids[fid]:
                    face_merged_vids[fid].discard(cur_vid)
                    face_merged_vids[fid].add(target)
                if cur_vid in face_extra_vids[fid]:
                    face_extra_vids[fid] = [
                        target if v == cur_vid else v
                        for v in face_extra_vids[fid]
                    ]
            for idx in range(len(all_pts)):
                if pt_to_vertex.get(idx) == cur_vid:
                    pt_to_vertex[idx] = target
            if any(fid in DEBUG_FACES for fid in valid_labels
                   if target in face_merged_vids[fid]):
                logger.debug("[MERGE] stage=9b-global-2px | vid %d -> %d dist=%.2f",
                             cur_vid, target, merge_dist)
            del vertices[cur_vid]
    ...
    for face, vert in face_vertices.items():
        logger.debug("face %d -> vids %s", face, vert)
    ...
    logger.info("Stage 4 — %d vertices, %d referenced by %d faces | face vertex count min/med/max = %d/%d/%d",
                len(vertices), len(vids_seen), len(face_vertices),
                min(face_vid_counts) if face_vid_counts else 0,
                int(np.median(face_vid_counts)) if face_vid_counts else 0,
                max(face_vid_counts) if face_vid_counts else 0)
    logger.info("Stage 4 — born-per-stage: %s",
                ", ".join(f"{k}={v}" for k, v in sorted(born_by_stage.items())))
    if under_three:
        logger.warning("Stage 4 — faces with fewer than 3 vertices: %s", under_three)

    # Sort each face's vertices by angle around centroid for consistent winding.
    for fid, vids in face_vertices.items():
        if len(vids) >= 3:
            coords = [vertices[vid] for vid in vids]
            avg_r = sum(v[0] for v in coords) / len(coords)
            avg_c = sum(v[1] for v in coords) / len(coords)
            face_vertices[fid] = sorted(vids, key=lambda vid: math.atan2(vertices[vid][0] - avg_r, vertices[vid][1] - avg_c))
...
def filter_edge_faces(vertices, face_vertices, valid_labels, face_pixels,
    ...
    logger.info("Stage 5 — dropped %d edge vertices and %d faces → %d faces, %d vertices remain",
                len(bad_vids), len(bad_faces), len(new_valid_labels), len(new_vertices))
    if bad_faces:
        logger.info("Stage 5 — dropped face ids: %s", sorted(bad_faces))
...
def write_nas(vertices, face_vertices, output_path, file_id=101):
    ...
    for fid in sorted(face_vertices):
        ...
        n = len(vids)
        if n not in (3, 4):
            logger.warning("Skipping face %d: has %d vertices (expected 3 or 4)", fid, n)
            continue
    ...
    logger.info("NAS — wrote %d nodes, %d elements → %s",
                len(vid_to_nid), elem_id - 1, output_path)
    return content
...
def extract_scale_reference(img, vertical_units=75.0, horizontal_units=100.0):
    ...
    logger.info("Scale — origin=(%.1f,%.1f) | horiz=%.2fpx/%.0fu | vert=%.2fpx/%.0fu | px_per_x=%.4f px_per_y=%.4f",
                origin_rc[0], origin_rc[1],
                horizontal_px, horizontal_units,
                vertical_px, vertical_units,
                px_per_x, px_per_y)
...
def detect_bulb_corners(img, brightness_thresh=240, min_size=5, max_size=200):
    ...
    logger.info("Stage 0 — %d bulbs detected (brightness>=%d, size %d–%d) | rejected: %d too-small, %d too-large",
                len(corners), brightness_thresh, min_size, max_size,
