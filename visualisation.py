...
def plot_corners(img, face_corners, face_pixels, valid_labels, save_path="corners.png"):
    ...
    for idx, (face_id, corners) in enumerate(face_corners.items()):
        ...
        # Left panel
        axes[0].scatter(corners[:, 1], corners[:, 0], c=[color], s=50, zorder=5)
        axes[0].add_patch(Polygon(corners[:, ::-1], fill=False, edgecolor=color, linewidth=2))
...
def plot_adjacency(img, labeled_clean, valid_labels, face_corners,
    ...
    axes[0].imshow(img)
    axes[0].set_title(f"Face adjacency graph: {G.number_of_edges()} edges")
    nx.draw(G, pos={i: (face_centroids[i][1], face_centroids[i][0]) for i in valid_labels},
            ax=axes[0], with_labels=True, node_color='yellow',
            edge_color='red', node_size=300, font_size=8, width=2)
...
def plot_merged_vertices(img, face_corners, all_pts, vertices,
    ...
    for pt in all_pts:
        ax.scatter(pt[1], pt[0], c='blue', s=30, zorder=4)
    ...
    for vid, pos in vertices.items():
        ax.scatter(pos[1], pos[0], c='red', s=60, zorder=6)
